...
/// Qihang AI Coding Assistant 代理服务器当前监听的端口
static CC_SWITCH_PROXY_PORT: OnceCell<RwLock<u16>> = OnceCell::new();

/// 设置 Qihang AI Coding Assistant 代理服务器的监听端口
///
...
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = CC_SWITCH_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated Qihang AI Coding Assistant proxy port to {port}");
        }
    } else {
        let _ = CC_SWITCH_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized Qihang AI Coding Assistant proxy port to {port}");
    }
}

/// 获取 Qihang AI Coding Assistant 代理服务器的监听端口
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 检查是否指向 Qihang AI Coding Assistant 自己的代理端口
    // 只有指向自己的代理才需要跳过，避免递归
    ...
    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 Qihang AI Coding Assistant 的端口时才返回 true
...
mod tests {
    ...
    fn test_build_client_direct() {
        let result = std::panic::catch_unwind(|| build_client(None));
        match result {
            Ok(inner) => assert!(inner.is_ok()),
            Err(_) => {
                #[cfg(target_os = "macos")]
                {
                    // macOS 无图形/受限测试环境下，系统代理查询可能在底层库触发 panic。
                    // 这里跳过该环境相关噪声，其他平台仍保持严格断言。
                    eprintln!("skip test_build_client_direct: system proxy API panic on macOS");
                    return;
                }
                #[cfg(not(target_os = "macos"))]
                panic!("build_client(None) should not panic");
            }
        }
    }
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 Qihang AI Coding Assistant 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 Qihang AI Coding Assistant 自己端口的 loopback 地址才返回 true
    ...
    fn test_system_proxy_points_to_loopback() {
        ...
        // 设置 Qihang AI Coding Assistant 代理端口
        set_proxy_port(15721);
        ...
        // 指向 Qihang AI Coding Assistant 端口的代理应该被跳过
